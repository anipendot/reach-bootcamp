...
(async () => { const startingBalance = stdlib.parseCurrency(10);
  ...
  const Player = (Who) => ({ ...stdlib.hasRandom, getFingers: async () => {
    const fingers = Math.floor(Math.random() * 6);
    console.log(`--------------------`); 
    console.log(`${Who} shoots ${FINGERS[fingers]} fingers.`);
      if (Math.random() <= 0.01 ) {
        for (let i = 0; i < 10; i++ ) {
          console.log(`${Who} is taking time to send it back.`);
          await stdlib.wait(1);
        }
      }
      return fingers;
  ...
  seeWinning: (winningNumber) => {
    console.log(`Total fingers thrown: ${winningNumber}`);
  },
